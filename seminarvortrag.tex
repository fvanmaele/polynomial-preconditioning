...
E20R0100 after applying a degree 10 polynomial is illustrated in Fig.
\ref{fig:spec_e20r0100}. 
...
using GMRES(20), reaching a tolerance of $\varepsilon=10^{-8}$ in
18193 and 258 steps, respectively. Preconditioning further increased
the convergence rate, reaching convergence in 1786 and 60 steps, respectively,
for a degree 3 polynomial. This is illustrated in Fig. \ref{fig:bidiag2}
for BiDiag2.
\item \textbf{S1RMQ4M1} did not converge with GMRES(50). With 20,000 steps
available, a polynomial of degree 5 was required to converge (in 11901
steps). Higher degree polynomials lead to better results. See Fig.
\ref{fig:convergence_s1rmq4m1}. The \textbf{Circle Eigenvalue Matrix}
behaved similarly, requiring a degree 3 polynomial to converge in
12564 steps.
\item \textbf{E20R0100} is a more difficult problem, and required a degree
7 polynomial to converge (in 9500 steps) with GMRES(100). The problem
...
up to \textasciitilde 6x (for the Circle Eigenvalue Matrix). For
E20R0100, there was up to \textasciitilde 8x increase, but with fast
convergence of a degree 10 preconditioner, latter only saw an increase
of \textasciitilde 1.25x in SpMVs. See Table \ref{tab:SpMVs} for
an overview.
...
\begin{figure}[tb]
\begin{centering}
\includegraphics[width=0.4\textwidth]{example_3_gmres}
\par\end{centering}
\caption{Convergence of BiDiag2, GMRES(20)\label{fig:bidiag2}}
\end{figure}
...
taken for problems such as SHERMAN5, where effectiveness varies for
different samples of the vector $v_{0}$, and can even result in worse
convergence.
