function [x, error, iter, flag, resvec, spmvs, inps] = gmresArnoldi( A, x, b, p, restart, max_it, tol )
...
%         resvec   REAL relative error norm in each step
    ...
    %r = M \ ( b-A*x );
    [r, mp] = applyPolynomial(p, A, b-A*x); % xxx: generalize: function handle for M\X
    spmvs = spmvs + 1 + mp;
    error = norm( r ) / bnrm2;
    if ( error < tol ) 
        return
...
%     for iter = 1:max_it                            % begin iteration
    for outer = 1:max_it
        ...
        for i = 1:m                                  % construct orthonormal
            ...
            [w, mp] = applyPolynomial(p, A, A*V(:,i));
            spmvs = spmvs + 1 + mp;
